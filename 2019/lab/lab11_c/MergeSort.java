...
public class MergeSort {
    ...
    private static <Item extends Comparable> Queue<Item> mergeSortedQueues(Queue<Item> q1, Queue<Item> q2) {
        ...
        while (true){
            if(q1.isEmpty() && !q2.isEmpty()){
                ...
                Item temp1 = q1.peek();
                Item temp2 = q2.peek();
                Item toEnqueue = temp1.compareTo(temp2) < 0 ? q1.dequeue() : q2.dequeue();
                result.enqueue(toEnqueue);
            }
            else
                break;
        }
    ...
    public static <Item extends Comparable> Queue<Item> mergeSort(Queue<Item> items) {
        ...
        Queue<Item> leftPart = new Queue<>();
        Queue<Item> rightPart = new Queue<>();
        int size = items.size();
        for(int i = 0; i < size / 2; i ++){
            leftPart.enqueue(items.dequeue());
        }
        while(!items.isEmpty()){
            rightPart.enqueue(items.dequeue());
        }
