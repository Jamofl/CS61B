...
public class BST<Key extends Comparable<Key>> {
    ...
    public double getAveDepth(){
        int N = this.size();
        return getTotalDepth(root) / size();
    }

    public double getTotalDepth(Node root){
        return getTotalDepth(root, 0);
    }

    private double getTotalDepth(Node root, int depth){
        if(root == null)
            return 0;

        else{
            return depth + getTotalDepth(root.left, depth + 1) + getTotalDepth(root.right, depth + 1);
        }
    }
    /**
