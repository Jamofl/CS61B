...
public class QuickSort {
    ...
    private static <Item extends Comparable> void partition(Queue<Item> unsorted, Item pivot,
        ...
        while( !unsorted.isEmpty()){
            Item temp = unsorted.peek();
            if(temp.equals(pivot))
    ...
    public static <Item extends Comparable> Queue<Item> quickSort(
        ...
        if(items.size() <= 1)
            return items;
        Queue<Item> smaller = new Queue<>();
        Queue<Item> equal = new Queue<>();
        Queue<Item> bigger = new Queue<>();

        partition(items, getRandomItem(items), smaller, equal, bigger);
        smaller = quickSort(smaller);
        bigger = quickSort(bigger);
        Queue<Item> result = catenate(smaller, equal);
        return catenate(result, bigger);
        // Your code here!
    }
}
